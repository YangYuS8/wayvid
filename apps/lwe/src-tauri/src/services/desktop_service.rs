...
impl DesktopService {
    pub fn load_page() -> Result<DesktopPageResult, String> {
        ...
        let (monitors, monitor_discovery_issue) = match monitors {
            MonitorDiscoveryResult::Known(monitors) => (monitors, None),
            MonitorDiscoveryResult::Unavailable { reason } => (Vec::new(), Some(reason)),
        };
        ...
        let stale = monitor_discovery_issue.is_some() || !assignments_available;
        ...
        match MonitorService::resolve_specific_monitor(&monitors, monitor_id) {
            ...
            MonitorDiscoveryResult::Known(_) => {
                match DesktopPersistenceService::save_assignment(monitor_id, item_id) {
                    ...
                    DesktopPersistenceWrite::Cleared => Ok(DesktopApplyResult::Unavailable {
                        reason: "Desktop apply returned a clear result".to_string(),
                    }),
                    DesktopPersistenceWrite::Unavailable { reason } => {
                        Ok(DesktopApplyResult::Unavailable { reason })
                    }
                }
            }
        ...
        match MonitorService::resolve_specific_monitor(&monitors, monitor_id) {
            ...
            MonitorDiscoveryResult::Known(_) => {
                match DesktopPersistenceService::clear_assignment(monitor_id) {
                    ...
                    DesktopPersistenceWrite::Saved => Ok(DesktopApplyResult::Unavailable {
                        reason: "Desktop clear returned a save result".to_string(),
                    }),
                    DesktopPersistenceWrite::Unavailable { reason } => {
                        Ok(DesktopApplyResult::Unavailable { reason })
                    }
...
mod tests {
    ...
    fn desktop_apply_flow_apply_to_monitor_returns_unavailable_when_dependencies_are_unavailable() {
        ...
        assert!(matches!(result, DesktopApplyResult::Unavailable { .. }));
    }
