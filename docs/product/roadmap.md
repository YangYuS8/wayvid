...
- `desktop-shell-and-library-flow`: the active `LWE` shell now has a monitor-aware apply/clear contract and truthful degraded-state handling, while follow-on work is still needed to replace the current structured-but-unavailable monitor discovery and persistence contracts with real system-backed implementations
