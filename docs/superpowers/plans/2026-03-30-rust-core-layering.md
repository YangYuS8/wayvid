...
The active layering surface in this plan stops at the `lwe-app-shell` <-> `wayvid-library` <-> `wayvid-core` <-> `wayvid-engine` boundaries. It does not introduce legacy GUI or CLI peers into the current workspace design.
