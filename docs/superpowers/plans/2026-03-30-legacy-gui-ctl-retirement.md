...
- rename `lwe-core`, `lwe-library`, or `wayvid-engine`
...
```md
Keep the active crate boundaries as `lwe-app-shell`, `lwe-library`, `lwe-core`, and `wayvid-engine` for now. The retired legacy crates `wayvid-gui` and `wayvid-ctl` are outside the active workspace path.
```
