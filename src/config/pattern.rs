...
/// # Examples
/// ```
/// use wayvid::config::pattern::matches_pattern;
///
...
mod tests {
    ...
    fn test_common_patterns() {
        // Common monitor naming patterns
        assert!(matches_pattern("eDP-1", "eDP-*"));
        assert!(matches_pattern("HDMI-A-1", "HDMI-A-*"));
        assert!(matches_pattern("DP-1", "DP-?"));
        assert!(matches_pattern("DVI-I-1", "DVI-*"));

        // Wayland output names
        assert!(matches_pattern("XWAYLAND0", "XWAYLAND*"));
        assert!(matches_pattern("WL-1", "WL-*"));
    }
}
